...
{
    public function rank(Collection $diagnoses, float $confidenceThreshold): Collection
    {
        $repeated = $diagnoses->groupBy(fn ($item) => $item->user_id.'|'.$item->predicted_class)->map->count();

        return $diagnoses->map(function ($diagnosis) use ($confidenceThreshold, $repeated) {
            ...
            $score = ($requested ? 100 : 0)
                + ($diagnosis->confidence < $confidenceThreshold ? 50 + ($confidenceThreshold - $diagnosis->confidence) : 0)
                + max(0, $repeatCount - 1) * 15;
            $reasons = collect([
                $requested ? 'Farmer requested review' : null,
                $diagnosis->confidence < $confidenceThreshold ? 'Low confidence' : null,
