<x-public-layout title="Privacy policy">
    ...
    <p>Effective date: September 3, 2026</p>
    ...
    <p>The mobile app's normal synchronization sends classification metadata, not the leaf image. Research contribution is optional and only applies when an explicit image-contribution control is presented and you choose it. Consented images still require agricultural review and a separate dataset-candidate decision before they can be approved for training use. The consent version and date are recorded with the image.</p>

    <h2>Storage and deletion</h2>
    <p>Mobile history is stored on your device and synchronized records are stored by the DahonMD server. You can delete individual history records in the app, delete your signed-in account from the account controls in the mobile workspace, or use the <a href="{{ route('account-deletion') }}">account deletion page</a>. Account deletion removes the account, access tokens, synchronized classifications, and their stored server images. When deletion is started from the signed-in mobile controls, the app also removes records and image copies linked to that account from the device. Device-only scans that were never linked to the account remain under your control and can be deleted individually, by clearing the app's data, or by uninstalling it.</p>
