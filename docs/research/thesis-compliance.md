...
| 15 | Offline stateless app | ❌ Release not verified | The active dependency graph is stateless and network-free, but classification cannot complete without the missing model asset. `check-release-readiness.mjs` fails closed. | `mobile-frontend/src/app/App.tsx` `mobile-frontend/scripts/check-release-readiness.mjs` |
...
| 17 | No accounts/history | ✅ Verified | `App.tsx:21`: explicit "No account, Internet connection, upload, or scan history is required." Active source imports no authentication, database, history, or synchronization module. | `mobile-frontend/src/app/App.tsx:21` |
