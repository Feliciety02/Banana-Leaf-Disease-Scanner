...
> The dated architecture audit in `docs/archive/audits/architecture-audit-2026-08-28.md` is a historical snapshot. Current runtime boundaries are documented in `docs/architecture/overview.md`.
...
| 2 | No Moko production class | ✅ Verified | `"moko": None`, `"moko-disease": None`, `"moko_disease": None` — all map to `None` (excluded). Config validation raises if a source alias becomes a class. | `ai/config/labels.py:43-45` `ai/config/config.py:216-223` |
...
| 13 | INT8 TFLite | ⚠️ Exported and audited; not the deployed format | `convert_tflite.py`: `target_spec.supported_ops = [TFLITE_BUILTINS_INT8]`, `inference_input_type = tf.int8`, `inference_output_type = tf.int8`. `quantization_audit.py`: checks `input_dtype_int8`, `output_dtype_int8`, `no_floating_point_tensors`, `full_integer_verified`. Locked-test benchmark shows full-integer INT8 drops accuracy 0.96644 → 0.84433, so the phone deploys FP32 and INT8 recovery is QAT future work. | `ai/deployment/convert_tflite.py:70-82` `ai/deployment/quantization_audit.py:30-56` `..._tflite/int8_evaluation.json` |
| 14 | Working native Android inference | ⚠️ Source implemented and model bundled; device run pending | `DahonMDTFLiteModule.kt` contains local model loading, exact float32 tensor checks, and mutex-serialized inference. The validated `ca_mobilenetv3_small_fp32.tflite` asset is bundled; the Android device path has not yet run. | `modules/dahonmd-tflite/android/.../DahonMDTFLiteModule.kt` `modules/dahonmd-tflite/index.ts` `mobile-frontend/src/features/classification/inference.ts` |
| 15 | Offline inference boundary | ✅ Asset bundled; device acceptance pending | Classification remains local and network-free, and the validated model is now bundled so the release gate passes. Physical-device offline acceptance still needs a representative Android run. Persistence and sync execute only after a genuine local result. | `mobile-frontend/src/features/classification/inference.ts` `mobile-frontend/scripts/check-release-readiness.mjs` |
| 16 | No inference backend dependency | ✅ Verified with scoped boundary | The optional workspace uses Laravel, but the classification module imports no HTTP, auth, database, sync, or connectivity service. The release gate scans that boundary directly. | `mobile-frontend/src/features/classification/` `mobile-frontend/scripts/check-release-readiness.mjs` |
| 17 | Account-free classification | ✅ Verified with optional history | Scanning and inference require no account or Internet connection. The product stores post-inference history locally and offers opt-in account synchronization; these features are not inputs to classification. | `mobile-frontend/src/app/App.tsx` `mobile-frontend/src/storage/localDiagnoses.ts` |
...
| 21 | Documentation consistent with implementation | ✅ Verified | Root, mobile, and architecture documentation distinguish the stateless thesis client from legacy/demo server-backed utilities, and state that the bundled production model is FP32 with the INT8 limitation disclosed. | `README.md` `mobile-frontend/README.md` `MODEL_EXPERIMENTS.md` `docs/architecture/overview.md` |
...
| Item | Status | Note |
|---|---|---|
| Dataset present and split | ✅ Complete | `banana-leaf-thesis-split-2878-v1` across PILOT-01–08 |
| Trained model files generated | ✅ Complete | PILOT-06 seed 42 `.keras` + FP32/INT8 `.tflite` exports |
| `ca_mobilenetv3_small_fp32.tflite` bundled | ✅ Complete | Production FP32 model bundled; release gate passes |
| On-device offline test and benchmark not run | ❌ Verification blocker | Phase 12b requires representative Android hardware |
| Davao field-subset evaluation | ⚠️ Pending | `PENDING EXPERIMENTAL VALIDATION` until field records are validated |
