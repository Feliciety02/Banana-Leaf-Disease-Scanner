...
const dbPromise = SQLite.openDatabaseAsync('bananacare-field.db');
...
export async function listCachedDiseases(): Promise<Disease[]> {
  ...
  return rows.map((row) => JSON.parse(row.payload) as Disease);
}
...
export async function listDiagnoses(ownerId: number): Promise<Diagnosis[]> {
  ...
  const rows = await db.getAllAsync<{
    ...
    image_uri: string | null; model_version: string; diagnosed_at: string; synced: number;
  ...
  return rows.map((row) => ({
    ...
    imageUri: row.image_uri, modelVersion: row.model_version, diagnosedAt: row.diagnosed_at, synced: row.synced === 1,
  }));
}
...
export async function saveDiagnosis(diagnosis: Diagnosis) {
  ...
  await db.runAsync(
    `INSERT OR REPLACE INTO diagnoses
      (id, owner_id, disease_id, confidence, latency, image_uri, model_version, diagnosed_at, synced)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    diagnosis.id, diagnosis.ownerId, diagnosis.diseaseId, diagnosis.confidence, diagnosis.latency, diagnosis.imageUri,
    diagnosis.modelVersion, diagnosis.diagnosedAt, diagnosis.synced ? 1 : 0,
  );
}
