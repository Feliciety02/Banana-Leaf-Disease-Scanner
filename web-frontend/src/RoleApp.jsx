...
import { api, authenticate, getToken, logout, requestPasswordReset, setToken } from './services/api';
...
const GUIDE_MEDIA = {
  healthy: { images: ['/assets/disease-guide/healthy-1.jpg', '/assets/disease-guide/healthy-2.jpg', '/assets/disease-guide/healthy-3.jpg'], source: 'Banana Disease Recognition Dataset', sourceUrl: 'https://data.mendeley.com/datasets/79w2n6b4kf/1' },
  dead: { images: ['/assets/disease-guide/dead-1.jpg', '/assets/disease-guide/dead-2.jpg', '/assets/disease-guide/dead-3.jpg'], source: 'Banana Disease Recognition Dataset', sourceUrl: 'https://data.mendeley.com/datasets/79w2n6b4kf/1' },
  sigatoka: { images: ['/assets/disease-guide/sigatoka-1.jpg', '/assets/disease-guide/sigatoka-2.jpg', '/assets/disease-guide/sigatoka-3.jpg', '/assets/disease-guide/sigatoka-4.jpg', '/assets/disease-guide/sigatoka-5.jpg', '/assets/disease-guide/sigatoka-6.jpg'], source: 'Banana Disease Recognition Dataset', sourceUrl: 'https://data.mendeley.com/datasets/79w2n6b4kf/1' },
  'panama-disease': { images: ['/assets/disease-guide/panama-stages/01-early-margin-yellowing/panama-leaf-stage-1.jpg', '/assets/disease-guide/panama-stages/02-expanding-chlorosis/panama-leaf-stage-2.jpg', '/assets/disease-guide/panama-stages/03-widespread-yellowing/panama-leaf-stage-3.jpg', '/assets/disease-guide/panama-stages/04-advanced-edge-necrosis/panama-leaf-stage-4.jpg', '/assets/disease-guide/panama-stages/05-near-total-leaf-death/panama-leaf-stage-5.jpg'], source: 'Banana Leaves Imagery Dataset (educational references only)', sourceUrl: 'https://doi.org/10.5281/zenodo.7670326' },
  'cordana-leaf-spot': { images: ['/assets/disease-guide/cordana-1.jpg', '/assets/disease-guide/cordana-2.jpg', '/assets/disease-guide/cordana-3.jpg'], source: 'Banana Leaf Spot Diseases (BananaLSD) Dataset', sourceUrl: 'https://data.mendeley.com/datasets/9tb7k297ff/1' },
};
...
function FarmerHome({ user, records, online, navigate, onOpen }) {
  ...
  return <div className="role-stack">
    <section className="farmer-welcome">
      ...
      <p>Check a banana leaf or look up a symptom you have noticed.</p>
    </section>

    {!online && <section className="farmer-sync-banner offline">
      <CloudOff size={22} />
      <div><strong>You're offline</strong><p>You can still scan now. Connect before saving the result.</p></div>
    </section>}
    ...
    {records.length > 0 && <section className="panel farmer-recent">
      ...
      {records.slice(0, 4).map((record) => <RecentCard key={record.id} record={record} onOpen={onOpen} />)}
    </section>}
  </div>;
}
...
function FarmerHistory({ records, navigate, onOpen }) {
  ...
  return <div className="role-stack"><Heading title="Scan history" text="Your newest saved results appear first." /><section className="panel history-role-panel"><div className="friendly-filters"><label><Search size={18} /><input placeholder="Search scans" value={query} onChange={(event) => setQuery(event.target.value)} /></label><select aria-label="Filter by result" value={kind} onChange={(event) => setKind(event.target.value)}><option value="all">All results</option><option value="healthy">Healthy</option><option value="dead">Dead leaf</option><option value="disease">Possible disease</option></select><input aria-label="Filter by date" type="date" value={date} onChange={(event) => setDate(event.target.value)} /></div>{filtered.map((record) => <RecentCard key={record.id} record={record} onOpen={onOpen} />)}{!filtered.length && <Empty icon={History} title={records.length ? 'No matching scans' : 'No scans yet'} text={records.length ? 'Adjust your filters and try again.' : 'Scan your first leaf to start your history.'} action={!records.length ? () => navigate('/farmer/scan') : undefined} actionLabel="Scan a leaf" />}</section></div>;
}
...
function DiseaseGuide() {
  ...
  return <div className="role-stack">
    <Heading eyebrow="GUIDE" title="Banana leaf guide" text="Only source-verified records appear here." />
    {!items.length ? <Empty icon={BookOpen} title="DISEASE CONTENT PENDING" text="No source-verified disease guide records are available yet." /> : <section className="guide-role-layout">
      ...
      {selected && <article className="panel guide-role-detail">
        <header className="guide-detail-intro">
          <div>
            <span className="guide-verified-badge"><Check size={12} />Source-verified disease guide</span>
            <h2>{selected.name}</h2>
        ...
        {guideImages.length > 0 && <section className="guide-gallery-section">
          ...
          {media && <p className="guide-image-credit">Field examples from <a href={media.sourceUrl} target="_blank" rel="noreferrer">{media.source}</a>, <a href="https://creativecommons.org/licenses/by/4.0/" target="_blank" rel="noreferrer">CC BY 4.0</a>. Reference images are not diagnostic confirmation.</p>}
        </section>}
