...
const ADMIN_NAV = [['/admin/dashboard', 'Dashboard', Home], ['/admin/accounts', 'Accounts', Users], ['/admin/diagnoses', 'Diagnoses', ScanLine], ['/admin/diseases', 'Disease Knowledge', BookOpen], ['/admin/sources', 'Research Sources', Link2], ['/admin/analytics', 'Analytics', BarChart3], ['/admin/model-comparison', 'Model Comparison', GitCompareArrows], ['/admin/system', 'Model Information', Settings], ['/admin/profile', 'Profile', CircleUserRound]];
...
function ModalShell({ open, title, description, onClose, children, size = 'medium', variant = 'modal' }) {
  ...
  if (!open) return null;
  return <div className={`crud-overlay ${variant}`}>
    <button type="button" className="crud-scrim" aria-label={`Dismiss ${title}`} onClick={onClose} />
    <section ref={panel} className={`crud-dialog ${variant} ${size}`} role="dialog" aria-modal="true" aria-label={title}>
      <header className="crud-dialog-header"><div><h2>{title}</h2>{description && <p>{description}</p>}</div><IconButton label={`Close ${title}`} onClick={onClose}><X size={20} /></IconButton></header>
...
function AuthPanel({ mode, onAuthenticated, onMode }) {
...
function FarmerHome({ user, records, online, navigate, onOpen, pendingChanges, syncing, onSync }) {
  ...
  return <div className="role-stack">
    ...
    {online && <section className="farmer-sync-banner">
      ...
      <button className="secondary-button" disabled={syncing} onClick={onSync}><RefreshCw className={syncing ? 'spin' : ''} size={16} />{syncing ? 'Syncing…' : 'Sync now'}</button>
    </section>}
...
function DiagnosisDialog({ record, onClose, onRequestReview, onDelete }) {
...
function AccountsAdmin({ initialRole = 'farmer' }) {
  ...
  const reviewer = role === 'agricultural_expert';
  ...
  return <div className="role-stack">
    <Heading eyebrow="ADMINISTRATION" title="Accounts" text="Farmers can register themselves. Administrators create reviewer accounts only after qualifications are confirmed." action={<button className={reviewer ? 'primary-button' : 'secondary-button'} onClick={create}><Plus size={17} />Add {singular}</button>} />
...
function DiagnosesAdmin() {
  ...
  const load = useCallback(() => { const query = new URLSearchParams(Object.entries(appliedFilters).filter(([, value]) => value)); query.set('per_page', '50'); api(`/admin/diagnoses?${query}`).then((payload) => { setItems(payload.data.items); setError(''); }).catch((exception) => setError(exception.message)); }, [appliedFilters]); useEffect(() => { load(); }, [load]);
...
export default function RoleApp() {
  const first = window.location.pathname; const [path, setPath] = useState(first); const [user, setUser] = useState(null); const [loading, setLoading] = useState(Boolean(getToken())); const [online, setOnline] = useState(navigator.onLine); const [records, setRecords] = useState([]); const [selected, setSelected] = useState(null); const [syncing, setSyncing] = useState(false); const [pendingChanges, setPendingChanges] = useState(0);
  ...
  const resetSessionUi = () => { setUser(null); setRecords([]); setPendingChanges(0); navigate('/', true); };
  const signedOut = async () => {
    ...
    if (remainingPending > 0 && !window.confirm(`${remainingPending} unsynchronized change${remainingPending === 1 ? '' : 's'} will be removed from this browser when you log out. Continue?`)) return;
    try {
      await logout();
    } catch {
      window.alert('DahonMD could not securely end the server session. Check your connection and try again.');
      return;
    }
  ...
  const syncNow = async () => { setSyncing(true); try { await loadRecords(); } finally { setSyncing(false); } };
